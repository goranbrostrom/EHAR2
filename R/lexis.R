lexis <- function(age = c(50, 100), cal = c(1829, 1895)){
    ...
    ## The 'romb' (not now):
    ##diag(c(1716, 30), 20, lty = 2, col = "red")
    ...
    diag(c(birth.min, 0), age[2], lty = 3, lwd = 0.9)
    ## 'Cohort' help line, end of period:
    diag(c(birth.max, 0), age[1], lty = 3, lwd = 0.9)
    ##
    abline(h = 0)
    ...
    diag(start, years, lty = 1)
    endp <- start + years
    text(endp[1] + 2, endp[2] + 2, "c", cex = 1.5)
    ####
    ## Third life line:
    start <- c(birth.min + 60, 0)
    years <- age[1]
    diag(start, years, lty = 2)
    start <- start + years
    years <- age[2] - years - 11
    diag(start, years, lty = 1)
    endp <- start + years
    text(endp[1] + 2, endp[2] + 2, "+", cex = 1.5)
    ####
