...
The \emph{log-rank test} \index{Tests!log-rank} is a $k$-sample test
...
\end{center}
\end{figure}
We are interested in investigating whether {\tt letters} and {\tt numbers}
...
for each observed \emph{event time}\index{event time}, see
...
\fun{coxph}\index{Functions!\fun{coxph}} function in the
...
The answer to that question is found five rows up, where \code{sexfemale}
reports the value 0.8245 for {\tt exp(Coef)}. The interpretation of this is
...
throughout the book. 
Note also that the grouping factor (\code{sex}) is given through the
...
which is a 
\code{factor} with three levels, instead of \var{sex}.
<<lrplace3>>=
require(eha) # Loads 'survival' as well.
data(oldmort)
...
summary(fit)
@ 
The degrees of freedom for the \emph{score test}\index{Tests!score}
is now $2$, equal to the number
of levels in \var{birthplace} (\code{parish}, \code{region}, \code{remote})
