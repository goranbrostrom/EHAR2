...
`oldmort` in `eha`. See Table \@ref(tab:lrreal3dat) for a sample of six records with selected columns. 
...
knitr::kable(head(om[-(1:1000), c("id", "enter", "exit", "event", "sex", "civ")]), booktabs=TRUE, caption = "Old age mortality.", row.names = FALSE)
```
...
```{r phazom3, fig.cap = "Old age mortality, women vs. men, cumulative hazards."}
par(las = 1)
...
set, `oldmort`, as above, but with the covariate `birthplace`,
which is a 
`factor` with three levels, instead of `sex`.
```{r lrplace3}
require(eha) # Loads 'survival' as well.
fit <- coxph(Surv(enter, exit, event) ~ birthplace,  data = oldmort)
summary(fit)
```
