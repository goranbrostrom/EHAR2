...
plot(males$age[1:101], males$mort[1:101] / females$mort[1:101], type = "l",
     ylim = c(0.9, 3), xlab = "Age", ylab = "Hazard ratio, men by women")
abline(h = 1, lty = 2)
```
...
library(eha)
fit <- logrank(Surv(enter, exit, event) ~ sex, data = om)
```
...
`r fit$p.value` , a very small number. Thus, there is a very significant difference in mortality between men and women.
...
```
Surv(enter, exit, event) ~ strata(sex), 
              data = om, start.time = 60)
with(om, plot(Surv(enter, exit, event) , xlab = "Age", fun = "cumhaz")
```
