...
```
with(om, plot(Surv(enter, exit, event), strata = sex, xlab = "Age"))
```

Note that the grouping factor (`sex`) is given through the
argument `strata`. The result is shown in Figure \@ref(fig:phazom3).

```{r phazom3, fig.cap = "Old age mortality, women vs. men, cumulative hazards.", echo = FALSE, fig.height = 4}
par(las = 1)
##plot(survfit(fit))
plot(Surv(om$enter, om$exit, om$event), strata = om$sex, xlab = "Age", main = "")
```
...
Furthermoree, the investigation is limited to *male mortality*.
...
par(las = 1)
with(om[om$sex == "male", ], plot(Surv(enter, exit, event), strata = civ, main = "", xlab = "Age", ylab = "", las = 1))
```
...
\begin{eqnarray*}
\mathbf{x} = (0, 0, \ldots, 0) & \Rightarrow & \mbox{in
group 0} \\
\mathbf{x} = (1, 0, \ldots, 0) & \Rightarrow & \mbox{in
group 1} \\
\mathbf{x} = (0, 1, \ldots, 0) & \Rightarrow & \mbox{in
group 2} \\
\cdots & & \cdots \\
\mathbf{x} = (0, 0, \ldots, 1) & \Rightarrow & \mbox{in
group k}
\end{eqnarray*}
...
\begin{equation}
\hat{H}_0(t) = \sum_{j:t_j \le t} \frac{d_j}{\sum_{m \in R_j}
e^{(\mathbf{x}_m - \bar{\mathbf{x}})\hat{\boldsymbol{\beta}}}},
(\#eq:bashazeha)
\end{equation}
...
\begin{equation*}
\hat{H}(t; \mathbf{x}) = \hat{H}_0(t) e^{(\mathbf{x} - \bar{\mathbf{x}})\hat{\boldsymbol{\beta}}}.
\end{equation*}
...
om$farmer <- factor(om$farmer, labels = "no", "yes")
...
The sampling frame is a rectangle in the Lexis diagram, see Figure \@ref(fig:lexmort).

```{r lexmort, fig.cap = "Old age mortality sampling frame"}
plot(c(1860, 1880), c(60, 60), type = "l", ylim = c(50, 100), axes = FALSE, lwd = 1.5)
lines(c(1860, 1880), c(85, 85), lwd = 1.5)
```
...
The concept of interaction is easiest to explain with two factors, see
Table \@ref(tab:twofactor).


           | No tumor | Small tumor     | Large tumor     
-----------|----------|-----------------|-----------------
Non-smoker | 1        | $\exp(\beta_1)$ | $\exp(\beta_2)$

<!--
\begin{table}[ht!]
  \tabletitle[Interaction, two factors]{ The role of interaction, two factors.}
\label{tab:twofactor}
\begin{tabular}{lllll}
   & & \multicolumn{3}{c}{\bf Tumor size} \\
   & & no tumor & small & large \\ \hline
\multicolumn{5}{c}{No interaction} \\
{\bf Smoking} & non-smoker & 1 & $e^{\beta_1}$ &   $e^{\beta_2}$ \\
\bf habits   & smoker & $e^{\alpha_1}$ & $e^{\alpha_1 + \beta_1}$ &
$e^{\alpha_1 + \beta_2}$ \\
%\end{tabular}
%\vspace{\baselineskip}
\multicolumn{5}{c}{With interaction} \\
%\begin{tabular}{lllll}
%   & & \multicolumn{3}{c}{\bf Tumor size} \\
%   & & no tumor & small & large \\ \hline
{\bf Smoking} & non-smoker & 1 & $e^{\beta_1}$ &   $e^{\beta_2}$ \\
...
With no interaction, the model is called *multiplicative*. The reason
is that with no interactions, the (relative) effect of smoking is *the
  same for each level* of tumor size, $\exp(\alpha)$, and vice
  versa. The model
with interaction is sometimes called *saturated*, and the reason is
that each combination of the two levels has its own parameter. Except for
the *reference* cell, to which all other effects are compared. 

In this example, two extra parameters, $\theta_{11}$ and $\theta_{12}$
measure interaction. If both are zero, we get the multiplicative model in
the upper part of Table \@ref(tab:twofactor).
