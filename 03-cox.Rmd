...
\begin{equation*}
\symbf{x} = (x_{1}, x_{2}, \ldots, x_{k}).
\end{equation*}
Then

\begin{eqnarray*}
\symbf{x} = (0, 0, \ldots, 0) & \Rightarrow & \mbox{in
group 0} \\
\symbf{x} = (1, 0, \ldots, 0) & \Rightarrow & \mbox{in
group 1} \\
\symbf{x} = (0, 1, \ldots, 0) & \Rightarrow & \mbox{in
group 2} \\
\cdots & & \cdots \\
\symbf{x} = (0, 0, \ldots, 1) & \Rightarrow & \mbox{in
group k}
\end{eqnarray*}
...
\begin{equation*}
h(t; \symbf{x}) = h_0(t) \prod_{\ell = 1}^k \psi_{\ell}^{x_{\ell}} =
\left\{ \begin{array}{ll}
  h_0(t), & \symbf{x} = (0, 0, \ldots, 0) \\
...
\begin{equation}
h(t; \symbf{x}) = h_0(t) e^{x_{1}\beta_1 + x_{2} \beta_2 + \cdots +
x_{k} \beta_k} = h_0(t) e^{\symbf{x} \boldsymbol{\beta}},
(\#eq:kgroups)
\end{equation}
...
$\symbf{x}_i$ take *any value*. Let data and model take
the following form:

**Data:** 

\begin{equation}
(t_{i0}, t_i, d_i, \symbf{x}_i), \; i = 1, \ldots, n,
\end{equation}
...
$\symbf{x}_i$ is a vector of *explanatory variables*.

**Model:**

\begin{equation}
h(t; \symbf{x}_i) = h_0(t) e^{\symbf{x}_i \boldsymbol{\beta}}, \quad t > 0.
(\#eq:pl)
\end{equation}

This is a *regression model* where the *response variable* is $(t_0, t, d)$ (we will call it a *survival object*)
and the *explanatory variable* is $\symbf{x}$, possibly (often) vector valued.
...
e^{\symbf{x}_m \hat{\boldsymbol{beta}}}},
(\#eq:bashaz)
\end{equation}
...
\begin{equation}
\hat{H}_0(t) = \sum_{j:t_j \le t} \frac{d_j}{\sum_{m \in R_j}
e^{(\symbf{x}_m - \bar{\symbf{x}})\hat{\boldsymbol{\beta}}}},
(\#eq:bashazeha)
\end{equation}

In order to calculate the cumulative hazards function for an individual
with a specific covariate vector $\symbf{x}$, use the formula

\begin{equation*}
\hat{H}(t; \symbf{x}) = \hat{H}_0(t) e^{(\symbf{x} - \bar{\symbf{x}})\hat{\boldsymbol{\beta}}}.
\end{equation*}

The corresponding survival functions may be estimated by the relation

\begin{equation*}
\hat{S}(t; \symbf{x}) = \exp\bigl(-\hat{H}(t; \symbf{x})\bigr)
\end{equation*}
...
\begin{example} Infant mortality and mother's age \end{example}
Consider a demographic example, the
influence of mother's age (a continuous covariate) on infant mortality.
