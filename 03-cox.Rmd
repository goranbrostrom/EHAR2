...
To summarize: The effects can be illustrated by two curves, one for women and one for men. The coefficients for `sex` (0 and 'r round(x[1, 1], 5)`) are the respective intercepts, and `r round(x[2, 1], 5)` and 
`r round(x[2, 1], 5)` +  `r round(x[3, 1], 5)`
