...
advantage is equally large in all ages. See Example \@ref(exm:sw192) for a
demonstration of this example of *proportional hazards*\index{proportional hazards}. 
...
expand the calendar time period to the 21st century, as far as we have data now.
The reason for this is the random fluctuations in low ages, caused by the 
extremely low mortality rates.

```{r twosex2, echo = FALSE, fig.cap = "Female and male mortality by age, Sweden 2001-2019.", fig.height = 3}
library(eha)
...
approaches to graphical presentation.
$\Box$
...
easier to see, the plot can be made on a *log scale* as in the right hand panel of 
Figure \@ref(fig:prophazfig3). Note that both dimensions are on a log scale. This type of plot,
...
`r fit$p.value` , a very small number. Thus, there is a very significant 
...
```{r logrank3, fig.cap = "Cumulative hazards for women and men in ages 60-85, 19th century Sweden.", echo = FALSE, fig.height = 4}
par(mfrow = c(1, 2), las = 1)
plot(fit, main = "Normal scale")
plot(fit, fn = "log", main = "Log scale", printLegend = "bottomright")
```


The proportionality assumption seems to be a good description from 60 to
85 years of age, but it is more doubtful in the very high ages (not shown). One
...
random fluctuations have a large impact in the high ages, but it is also an often 
observed phenomenon that differences in mortality between groups tend to vanish in
the extremely high ages.
...
is a special case of Cox regression, which will be described in
