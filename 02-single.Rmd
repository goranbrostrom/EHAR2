...
respectively. Both cover the years 1968--2019. We use both to construct a data frame
`females` containing population size and number of deaths for females 2019. 
...
```{r scbdeadfig, fig.cap="Female deaths by age, Sweden 2019.", echo = FALSE}
par(las = 1)
...
by dividing the number of deaths by the population size, age by age, so  we add 
two columns, derived from the original three, in the following way.
...
knitr::kable(fem, row.names = FALSE, booktabs = TRUE,
             caption = "Life table for Swedish women 2019.", align = "r")
```
...
```{r lifetablefig, fig.cap = "Life table and survival function, females, Sweden 2019.", fig.scap = "Life table and survival function, females, Sweden 2019.", echo = FALSE,fig.height=4,message=FALSE}
library(eha)
...
##plot(females$age, females$alive / 100000, type = "s",
  ##   xlab = "Age", ylab = "Surviving fraction",
    ## axes = FALSE)
axis(1, at = c(0, 50, 87.6, 100))
axis(2, at = c(0, 0.5, 1), las = 1)
box()
abline(v = 0)
abline(h = 0)
abline(h = 0.5, lty = 3)
abline(v = 87.588, lty = 3)
```

It makes sense to plot the life table, see Figure \@ref(fig:lifetablefig). 
Exactly how this is done will be shown later in the chapter. *Median life length* is
