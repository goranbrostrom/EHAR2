...
We start with a motivating example (\@ref(exm:sw16)), the life table, and its connection to
death risks and survival probabilities in a population, see 
Table \@ref(tab:lifewomen).

```{example, sw16, name = "Swedish life tables", echo = TRUE}
```
...
gender, and year. From such data it is possible to construct a *life
table*.
...
possible to *aggregate* later, if that is preferred.
...
respectively. Both cover the years 1968--2019.
...
The argument `stringsAsFactors = FALSE` is needed in order to avoid character columns
...
```{r scbdeaths, echo = TRUE}
deaths <- read.table("Data/BE0101D9.csv", header = TRUE,
                     stringsAsFactors = FALSE,
                     fileEncoding = "latin1")
```

The result is shown in Table&nbsp;\@ref(tab:scbdeathtab).
...
females$alive <- alive
```
The result is shown in Table&nbsp;\@ref(tab:lifewomen).

```{r lifewomen, echo = FALSE}
fem <- females
...
fem[6, ] <- rep("...", 5)
fem <- fem[c(1:6, 97:101), ]
## For text input:
d1 <- round(females$risk[1] * 100000)
l2 <- 100000 - d1
l2 <- as.character(l2, digits = 5)
knitr::kable(fem, row.names = FALSE,
             caption = "Life table for Swedish women 2016.", align = "r")
```
...
expect `r fem$risk[1]` x 100000 = `r d1` girls to die the
first year. In other words, 100000 - `r d1` =
`r l2` will remain alive
...
mortality conditions of the year 2016. Of course no real individual can
...
```{r lifetablefig, fig.cap = "Life table and survival function, females, Sweden 2016.", fig.scap = "Life table and survival function, females, Sweden 2016.", echo = FALSE,fig.height=4,message=FALSE}
library(eha)
...
##plot(females$age, females$alive / 100000, type = "s",
  ##   xlab = "Age", ylab = "Surviving fraction",
    ## axes = FALSE)
axis(1, at = c(0, 50, 87, 100))
axis(2, at = c(0, 0.5, 1), las = 1)
box()
abline(v = 0)
abline(h = 0)
abline(h = 0.5, lty = 3)
abline(v = 86.746, lty = 3)
```

It makes sense to plot the life table, see Figure \@ref(fig:lifetablefig). Exactly how this is done will be shown later in the chapter. *Median life length* is
`r round(qpch(0.5, 1:100, females$risk), 2)`, see construction in the figure.
By
...
reads *"the probability that $T$ is equal to or larger than
  $t$"*; here $t$ is a fixed number while $T$ denotes a "random
quantity". In statistical language, $T$ is called a *random variable*. In our example, $T$ is the (future) life length of a randomly
...
$$
This is illustrated in Figure \@ref(fig:densityfig), with $t_0 = 10$ and $s = 2$. The application is *remaining life at age 60* in a nineteenth century setting.
...
plot(x[1:85], f(x[1:85]), type = "l", axes = FALSE, ylim = c(-0, max(y) + 0.01), 
     xlab = "Remaining years after age 60.", ylab = "", xlim = c(0, max(x)))
...
\emph{conditional} probability\index{conditional probability} per time unit. 
<!--
Otherwise, the
...
```{r geom2,echo=FALSE,fig.height=5, fig.cap = "The geometric distribution with success probability 0.25.", fig.scap="The geometric distribution.",echo=FALSE}
source("R/plgeo.R")
par(las = 1)
...
*Nelson-Aalen* estimator of the cumulative hazards function, see the next
...
the `plot` function can be used to plot both Nelson-Aalen and
Kaplan-Meier
...
and the result is seen in Figure \@ref(fig:pshazsur), with 95 per cent pointwise confidence intervals.


```{r pshazsur,fig=TRUE,echo=FALSE,fig.cap="Nelson-Aalen plot (left panel) and Kaplan-Meier plot (right panel), male mortality data. The dashed lines in the Survival function plot are 95 percent confidence limits.", scap = "Male mortality, Nelson-Aalen and Kaplan-Meier plots.", fig.height = 3}
par(mfrow = c(1, 2)) 
...
The package `survival` must be loaded for this to work. Note the use of the
function `with`\index{Functions!`with`}; it tells the `plot`
function that it should get its data `(enter, exit, event)` from the
data frame `mort`. The function `Surv`\index{Functions!`Surv`}
...
function `phreg`\index{Functions!phreg|(} 
(it also works with the
function `aftreg`\index{Functions!aftreg}). Just 
...
par(las = 1)
plot(fit.w, fn = c("cum"), main = "", axes = FALSE)
axis(2, at = c(0.0, 0.1, 0.2, 0.3))
axis(1)
box()
plot(fit.w, fn = c("sur"), main = "")
```
