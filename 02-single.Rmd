...
```{example, sw16, name = "Swedish female and male life tables 2016."}
\index{life table|(}
...
```{r lifetablefig, fig.cap = "Life table and survival function, females, Sweden 2016.", fig.scap = "Life table and survival function, females, Sweden 2016.", echo = FALSE,fig.height=4}
library(eha)
...
It makes sense to plot the life table, see Figure&nbsp;\@ref(fig:lifetablefig). Exactly how this is done will be shown later in the chapter. _Median life length_ is
...
\begin{equation}
S(t) = P(T \ge t), \quad t > 0, \label{eq:surv}
\end{equation}
...
quantity". In statistical language, $T$ is called a *random 
...
\begin{equation}
f(t) = -\frac{d}{dt} S(t), \quad t > 0 \label{eq:dens}
\end{equation}
...
\begin{equation}
f(t) = \lim_{s \rightarrow 0} \frac{P(t \le T < t + s)}{s}, \quad t > 0. \label{eq:denslim}
\end{equation}
...
\begin{equation}\label{tab:estatoms}
\begin{split}
...
number
by the size of the corresponding risk set. The result is shown in  Table \@ref(tab:estatoms).
...
with(mort, plot(Surv(enter, exit, event), axes = FALSE, fn = "cum"))
axis(1)
...
The package `eha` must be loaded for this to work. Note the use of the
