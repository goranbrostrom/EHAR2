...
\begin{equation}
f(t) = -\frac{d}{dt} S(t), t > 0 \label{eq:dens}
\end{equation}
...
$$
This is illustrated in Figure&nbsp;\@ref(fig:densityfig).

```{r densityfig, fig.cap = "Interpretation of the density function.", echo=FALSE,fig.height=4}
x <- seq(0, 40, length = 100)
f <- function(x) dgompertz(x, scale = 18.9, shape = 1.45, param = "canonical")
plot(x, f(x), type = "l", axes = FALSE, ylim = c(-0, 0.1), 
     xlab = "Age", ylab = "", xlim = c(0, 40))
arrows(0, 0, 0, 0.1, length = 0.1)
arrows(0, 0, 40, 0, length = 0.1)
text(3, 0.08, "t")
text(0.14, 0.95, "f(t)")
ats <- c(0, 10, 12, 20, 30, 40)
axis(1, at = ats, labels = ats + 60)
axis(2)
x0 <- 10
x0s <- 12
...
f <- function(x) eha::hweibull(x, scale = 1, shape = 3)
plot(x, f(x), type = "l", axes = FALSE, 
     ylim = c(-0.5, 10), xlab = "", ylab = "", xlim = c(0,2))
arrows(0, 0, 0, 0.95, length = 0.1)
arrows(0, 0, 3, 0, length = 0.1)
text(3, 0.08, "t")
text(0.14, 0.95, "h(t)")
x0 <- 1
x0s <- 1.2
