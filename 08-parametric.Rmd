...
piecewise constant hazards model, where the former can be seen as a 
...
generated by varying $\lambda$ in equation \eqref{eq:6weibdist}. On the other hand,
if we pick two members from the family of distributions defined by equation 
\eqref{eq:6weibdist} with *different* values of $p$, they would not be proportional. 
...
The function `phreg` in package **eha** fits models given by equation
...
and `sex` with *female* as reference category.

```{r coxold8, echo = FALSE, results="asis"}
olm$sex <- relevel(olm$sex, ref = "female")
...
tbl(out, caption = "Coefficients with Cox and Weibull regressions, data oldmort.", 
    rownames = TRUE)
```
...
tbl(out, caption = "Coefficients with Gompertz, Cox and Weibull regressions, time scale duration since age 60.", rownames = TRUE)
```
...
##source("R/tbl_html.R")
tbl(yy, caption = "Coefficients with Gompertz, Cox and Weibull regressions, time scale age." ,
    rownames = TRUE)
```
...
source("R/unicode.R")
cap = "Table version of the data frame oldmort for the age interval 60--65 (age omitted)."
xx <- head(olmtab, 6)
xx$age <- iv(60, 65) # Cheating typography, fix in tpchreg?
tbl(xx, caption = cap)
```
...
tbl(xx, rownames = TRUE, caption = "Comparison of conditional and unconditional maximum likelihood values.")
``` 
...
capt <- "Old age (60+) mortality in 19th century Skellefteå, AFT model."
...
Note that the "Max. log. likelihood", `r um(maxml)`, is not exactly the same, and 
...
if the time span is cut in small enough pieces. So we cut the first eight years in half-year-long pieces, see Figure \@ref(fig:pchivl8).
...
lab2 <- "phlognorm28"
cap2 <- "Length of birth intervals, Lognormal AFT model, reverse parameterization."
...
log likelihoods: For the Lognormal it is `r um(round(fit.lognorm$loglik[2], 2))`, and for the 
Loglogistic it is `r um(round(fit.loglogist$loglik[2], 2))`, so it seems as if the 
...
*the canonical parameterization* in the package `eha`. It works as follows.
...
`r um(round(fit.g$loglik[2], 1))`, compared to
`r um(round(fit.gaft$loglik[2], 1))` for the latter. This indicates that the
...
The result is shown in Table \@ref(tab:mlreg8), and 
