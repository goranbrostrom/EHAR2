...
```{r aftph8,fig.cap="Proportional hazards (left) and accelerated failure time model (right). The baseline distribution is Loglogistic with shape 5 (dashed).",echo=FALSE}
...
capt <- "Old age mortality in 19th century Skellefteå, AFT model."
...
covariates, the AFT and the PH models are equivalent.
$\Box$
...
tbl(head(f12), caption = "The fertility data set, first few rows.")
```
...
right shape of the hazard function: The *Lognormal* and the *Loglogistic* distributions.
...
The interpretation of the regression coefficients are different from the PH case:
...
We repeat the PH analysis in Table \@ref(tab:gocofph8), but with the AFT model.

```{r gocofaft8, results='asis'}
fit.gaft <- aftreg(Surv(enter - 60, exit - 60, event) ~ sex + region, 
                   id = id, param = "lifeExp", dist = "gompertz", 
                   data = oldmort)
cap = "Old age mortality, Gompertz AFT model."
lab = "gocofaft8"
