...
is that the total elapsed time was around six tenth of a second. Quite OK for a
proportional hazards analysis involving `r sum(sw$deaths)` events.
...
```{r tpchtab7, results = 'asis'}
source("R/fit.out.R")
fit.out(fit, caption = "Swedish mortality, 1968-2020", label = "tpchtab7")
```

Note the "Restricted mean survival": It gives the expected remaining life 
...
tbl(rl, caption = "Register data, Sweden December 31, 2001.")
```
...
##     kable_styling(font_size = 10)
tbl(wl, caption = "Constructed data, Sweden December 31, 2001.")
```
...
tbl(lhead, caption = "Mortality in ages above 30, Sweden 2002 ('year' = 2001).", fs = 10)
```
...
weather conditions on mortality. Daily temperature data from
[SMHI](https://www.smhi.se/data/meteorologi/ladda-ner-meteorologiska-observationer#param=airtemperatureInstant,stations=all,stationid=140500) for central Umeå, starting at 1 
November, 1858 and ending at 30 September, 1979 (this particular weather station was replaced at this last date).
are used together with population data from Umeå, a town geographically located at the 63rd latitude in Sweden.
Since we attempt to match these data to population data for the time period 1 January 1901 -- 31 December 1950, we
 cut the temperature data to the same time period.
...
tbl(head(temp, 10), fs = 11, caption = "Temperature in Umeå, 1901-1950") # fs stands for 'font size'
```
...
As an example of combining the use of the function `eha::make.communal` and 
tabulation, as described in this chapter, the effect of hot summers on mortality 
is examined. Data at hand relate to Umeå during the time period 1990--2014.
Umeå is a town in the northern part of Sweden.
...
data for Umeå is read and presented i Table \@ref(tab:readtemp7).
...
library(kableExtra)
source("~/Documents/EHAR2/R/tbl.R")
tbl(head(temp, 5), caption = "First rows of temperature data for Umeå.")
```
...
```{r aggrtemp7, cache = TRUE}
aggrtemp <- aggregate(temp ~ month + year, data = temp, FUN = max) #day temp
aggrtemp$yearmonth <- 100 * aggrtemp$year + aggrtemp$month
head(aggrtemp[aggrtemp$month %in% 6:8,])
```

The next step is to aggregate the population data, but first must split it by 
...
out <- toTpch(Surv(enter, exit, event) ~ yearmonth + sex, data = compop, 
...
on high temperature (aove 29 degrees Celsius). The July maximum temperatures by year 
are shown in Figure \@ref(fig:julmaxtemp7).

```{r julmaxtemp7, fig.cap = "Maximum temperatures in July by year 1990-2014."}
with(aggrtemp[aggrtemp$month == 7, ], 
     plot(year, temp, type = "b", ylim = c(20, 32)))
abline(h = 29, lty = 3)
```


```{r julydata7, cache = TRUE, results='asis'}
