...
text(78, 0.3, "Expected life at birth (81.1)", srt = 90, cex = 0.8)
```
...
for (i in 1:4){
    ...
    y <- fit2$hazards[2, ] / fit2$hazards[1, ]
    plot(x, y, main = "", xlab = paste(start, "-", slut, sep = ""), ylab = "Hazard ratio", 
...
[SMHI](https://www.smhi.se/data/meteorologi/ladda-ner-meteorologiska-observationer#param=airtemperatureInstant,stations=all,stationid=140500) for central Umeå, starting at 1 November, 1858 and ending at 30 September, 
...
1 January 1901 -- 31 December 1950, we
...
atmp <- aggregate(list(temp = mtemp$maxtemp), 
                  ...
                  FUN = min)
tbl(head(atmp, 3), fs = 10, 
    caption = "Average daily maximum temperature by month, Umeå 1901-1950.")
```
...
tbl(head(nt, 3), fs = 11, 
    caption = "Population data split by year and month, Umeå 1901-1950.")
```
...
tbl(head(nt[, c("id", "sex", "enter", "exit", "event", "year", "month", "socStatus", "temp")], 3), 
    fs = 11, caption = "Temperature added to Umeå data.")
```
...
fit.out(fit, label = "coxph7", 
        caption = "Cox regression, mortality and temperature, Umeå 1901-1950.")
```
...
```{r tabulate7}
told <- toTpch(Surv(enter, exit, event) ~ sex + socStatus + yearmonth, 
...
tbl(head(told, 6), fs = 11, caption = "Tabular Umeå 1901-1950 data.")
```
...
library(kableExtra)
source("~/Documents/EHAR2/R/tbl.R")
tbl(head(temp, 5), caption = "First rows of temperature data for Umeå 1990-2014.")
```
