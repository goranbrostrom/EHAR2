...
country
```
The first level often have a special meaning ("reference category") in
...
above). This is often a sign of a mistake in the code.
...
relying (and remembering) those rules. Compare, for instance
```{r seqC}
1:3 + 1
1:(3 + 1)
```
...
Note that a dimension is lost when one row /column is selected. This can be
...
In serious function writing, it is very unconvenient to define the function
...
is covered later in these notes. Our favourite editor, and the one used in
examples in these notes, is `emacs`, which is free and available on all
platforms. 
...
*unspecified* (or both). Unspecified arguments are shown as "..."
in the function definition. See for instance the functions `c` and `min`.
The "..." may be replaced by any number of arguments in a call to the
function.
...
Below is a function tha^t calculates the maximum likelihood estimate of the
...
mlexp <- function(x, d){
    ...
    ## But we prefer a parametrization where the parameter may vary freely.
...
``` 
Note that the parantheses in the last line are strictly unnecessary, but
...
at the prompt (or, find a corresponding menu item on a GUI system). Then a
...
*   **age.window**\index{Functions!\fun{age.window}} Makes a "horizontal
  ...
  `Surv`\index{Functions!Surv} function. The default names are
...
187070, that is, from January 1, 1860 up to and including December 31, 1869.
...
*   **coxreg**\index{Functions!coxreg} Fits Cox proportional hazards
  model. It is a wrapper for `coxph` in case of simpler Cox regression
  models.  
  
*   **glmmML**\index{Functions!glmmML} Fits clustered binary and count
  data regression models.

*   **phreg**\index{Functions!phreg} Fits parametric proportional
  hazards models.
 
*   **risksets**\index{Functions!risksets} Finds the members of each
  riskset at all event times.



### Checking the the integrity of survival data

In data sources collected through reading and coding old printed
handwritten sources, many opportunities for making errors occur. Therefore,
...
job in the transfer of data from old churchbooks to digital data files. The
...
the priests are allowed; the digital files are supposed to be as close as
...
#### Old age mortality {# old-age-mortality13}
...
*Excel*) data. This is done with the aid of the package `foreign`.
