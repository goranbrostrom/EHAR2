...
from the "normal" as a proxy for *short-term variation in economic
  stress*. We illustrate the idea with a subset of the built-in data set `scania`.
...
Then we compose a table of the four results, see Table \ref(tab:5comt). 
...
```{r 5nalplot,fig=TRUE,echo=FALSE,caption="Nelson-Aalen plot with the aid of the function risksets"}
plot(rs$risktimes, cumsum(rs$n.event / rs$size), type = "s", 
     xlab = "Duration (years)", ylab = "Cumulative hazards")
abline(h = 0)
```
...
abline(h = 0)
```
and the result is shown in Figure \@ref(fig:5km)

```{r 5kmout,fig=TRUE,echo=FALSE,cap="Survival plot with the aid of the function risksets.",scap="Survival plot with the aid of the function risksets"}
sur <- exp(-cumsum(rs$n.event / rs$size))
plot(rs$risktimes, sur, type = "s", 
     xlab = "Duration (years)", ylab = "Surviving fraction")
abline(h = 0)
