...
\begin{example} {Mortality in ages 61--80, Sweden 2007} \end{example}
\label{ex:sw07}
In {\tt eha}, there is the data set {\tt swe07}, which contains population
...
population size for {\tt age} $i$ and {\tt sex} $j$, $i = 61, \ldots, 80$;
...
($\log(P_{ij}$) that is the correct \emph{offset} to use in the Poisson regression.
First we want {\tt age} to be a factor (no restrictions like linearity),
then the \R\ function {\tt glm} (``generalized linear model'') is used to
...
@
The function {\tt drop1} is used above to perform two LR tests. The results
...
and age. To know \emph{how}, we present the parameter estimates.
<<parest>>=
summary(fit)
@
...
surprises!). The parameter estimates for {\tt age} also opens up the
...
@
Note that the {\tt drop1} function only tests the interaction term. This is
...
we can conclude that in a survival model with {\tt sex} as covariate, we
have \emph{proportional hazards}! This is most easily seen in
\emph{graphs}. The first plot is based on the estimated Poisson model
...
abline(h = 0)
@
Note the parameter {\tt ylim}. It sets the scale on the $y$ axis, and some
trial and error may be necessary to get a good choice.
The function \fun{line}\index{Functions!\fun{line}} adds a curve to an already
existing plot, and the function \fun{abline}\index{Functions!\fun{abline}}
adds a straight line; the argument {\tt h = 0} results in a
\emph{horizontal} line. See the help pages for these functions for further
information. 
at 0. 
The result is shown in Figure~\ref{fig:poishaz}.
\begin{figure}[ht!]
\begin{center}
<<modfig,fig=TRUE,echo=FALSE>>=
plot(61:80, lambda.males, ylim = c(0, 0.06), xlab = "Age", 
     type = "s")
lines(61:80, lambda.females, type = "s", lty = 2)
abline(h = 0)
@
\caption[Hazard functions for males and females, model based]{Model based hazard functions for males (upper) and females (lower).}
\label{fig:poishaz}
\end{center}
\end{figure}

We can also make a plot based only on raw data. For that it is only
necessary to calculate the \emph{occurrence-exposure
...
\begin{figure}[ht!]
\begin{center}
<<rawplot,fig=TRUE,echo=FALSE>>=
plot(61:80, m.rate, ylim = c(0, 0.06), xlab = "Age", 
     ylab = "Mortality", type = "s")
lines(61:80, f.rate, type = "s", lty = 2)
abline(h = 0)
@
\caption[Hazard functions for males and females,
  raw data]{Hazard functions for males (upper) and females (lower) based on
  raw data.}
\label{fig:rawplot}
\end{center}
\end{figure}
...
without interaction fits the Swedish old age mortality data very well. \eex
\index{Distributions!Poisson|)}
