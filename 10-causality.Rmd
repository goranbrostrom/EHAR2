...
in a nineteenth century environment later in this chapter. There $A$
...
`par`\index{Functions!`par`}; the first call sets the plotting area
...
```{r aafig9,echo=FALSE,fig.cap="Cumulative intercept (left) and cumulative regression coefficient (right)."}
oldpar <- par(mfrow = c(1, 2))
plot(fit)
par(oldpar)
```
...
\begin{equation*}
dN(t) = \{\beta_0(t) + \beta_2(t) \theta_0(t) + \bigl(\beta_1(t) +
\theta_1(t) \beta_2(t)\bigr) X_1 + \beta_2(t) \epsilon(t)\}dt + dM(t)
\end{equation*}
...
`risksets`\index{Functions!`risksets`}  in `eha`;
it keeps track of the composition of the riskset over time, which is
exactly what is needed for implementing the dynamic path analysis.
...
half, which corresponds to $\beta = 0$ in \@ref(eq:9pair). 
...
tbl(head(infants, 5), caption = cap)
```
...
data = infants)
cap = "Infant mortality, statified Cox regression."
lab = "matchedanal10"
...
```
\index{Functions!`ifelse`} Note the use of the function `ifelse`:
...
different parametrizations. The second, with a centered mother's age, is
...
one must show (or assume) the there are "no unmeasured
