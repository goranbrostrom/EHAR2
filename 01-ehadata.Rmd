...
i.e., *event* is a *logical* variable taking the value `TRUE`
...
Unfortunately, you may still see articles where these two concepts are confused.
...
```{example, name = "Infant mortality", label = "infmort"}
```
...
```{example, name = "Old age mortality, tabular data", label = "oldagetab"}
```
...
ages 61 and above for the year 2007.
...
*   **log.pop** The natural logarithm of *pop*. This variable is used as
  *offset* in a Poisson\index{Distributions!Poisson} regression.

See Chapter 4 for how to analyze this data set.
\index{Data sets!tabular!old age mortality|)}$\Box$
