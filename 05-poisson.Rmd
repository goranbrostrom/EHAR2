...
```{example, name = "Mortality in ages 61--90, Sweden 2019-20."}
```
\label{ex:sw07}
...
mortality is lowered in all age groups from one year to the next, but 2019 to 2020 is different.
...
fit <- glm(deaths ~ offset(log(pop)) + year + sex + age, family = poisson, 
           data = swe)
...
```r
par(las = 1)
plot(ages, lambda.2019, ylim = c(0, 0.15), xlab = "Age", ylab = "Mortality",
     type = "S")
...
likelihood ratio implied, this finding is not statistically significant on the 5 percent
level.
...
in the **eha** package for a smooth and simple way of analysing tabular data.$\ \Box$
\index{Distributions!Poisson|)}
